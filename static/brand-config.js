(function () {
  window.SITE_BRAND = Object.freeze({
    masterBrand: "고주무관의 쉬운 학교행정",
    editionTitle: "문막초 교직원 안내",
    siteTitle: "고주무관의 쉬운 학교행정 | 문막초 교직원 안내",
    siteDescription: "문막초 교직원을 위한 쉬운 학교행정 안내 웹페이지",
    schoolName: "문막초등학교",
    schoolShortName: "문막초",
    operatorName: "고주무관",
    contactEmail: "edusproutcomics@naver.com",
    slogan: "교직원이 덜 헤매도록, 행정은 더 쉽게",
    homeHeadline: "문막초 교직원을 위한 쉬운 학교행정 안내",
    homeSubtitle: "문막초가 좋은 행정실 주무관 고주무관이, 자주 묻는 행정·회계 업무를 이해하기 쉽게 정리했습니다.",
    trustMessage: "이 웹페이지는 업무 참고용이며, 최종 기준은 공문·지침 등 공식 자료를 우선합니다.",
    affectionLine: "문막초를 더 편하게 돕기 위해 만든 개인 운영 페이지입니다.",
    siteIntroLines: [
      "문막초등학교 교직원을 위한 업무 참고용 안내 페이지입니다.",
      "자주 묻는 행정·회계 업무를 실무에 바로 쓰기 쉽도록 정리했습니다.",
      "최종 기준은 공문·지침 등 공식 자료를 우선합니다."
    ],
    playfulAlias: "업무천재 고주무관",
    allowPlayfulAliasIn: ["footer-small", "newsletter-corner", "easter-egg"],
    brandAccentColor: "#1f3a5f",
    schoolAccentColor: "#628a63",
    routes: {
      home: "/",
      howToUse: "/notice/how-to-use/",
      notice: "/notice/",
      biteGuide: "/a-bite/",
      legacyNewsletter: "/newsletter/",
      detailedGuide: "/guide/",
      tools: "/tools/",
      downloads: "/file-download/",
      legacyABite: "/a-bite/"
    },
    usePublicThemeToggle: false,
    lightOnly: true
  });
})();

...
(function () {
  ...
  if (window.__MM_GLOBAL_LOADER_LOADED__) return;
  window.__MM_GLOBAL_LOADER_LOADED__ = true;
  ...
  // -----------------------------
  // 9. 브랜드 UI 자동 로딩
  //    - Edusprout 정적 파일처럼 global-loader만 포함되어도
  //      style.css, brand-config.js, brand-ui.js가 순서대로 로드되게 함
  // -----------------------------
  const BRAND_STACK_VERSION = "20260426-header-title";
  ...
  function ensureStaticStyle() {
    ...
    link.rel = "stylesheet";
    link.href = "/static/style.css?v=" + BRAND_STACK_VERSION;
    document.head.appendChild(link);
  }

  function ensureStaticScript(src, attrs) {
    ...
    return new Promise(function (resolve) {
      ...
      script.src = src + "?v=" + BRAND_STACK_VERSION;
      script.async = false;
      ...
      script.onerror = function () { resolve(script); };
  ...
  function ensureBrandStack() {
    ...
    return ensureStaticScript("/static/brand-config.js").then(function () {
      return ensureStaticScript("/static/brand-ui.js", { defer: "" });
    });
  }
